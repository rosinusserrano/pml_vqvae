...
As such, our latents are of size $\mathbb{R}^{2\times 32 \times 32}$ when feeding in $3 \times 128 \times 128$
ImageNet images.
We therefore reach a compression down to compressed $4\text{ bits}\/\text{pixel}$, which is similar to the file size
at a compression rate of JPEG at 70\% quality~\cite{citationNeeded}.e of JPEG retaining 70\% of image quality.
